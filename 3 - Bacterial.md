...
```shell
sinteractive -c 28 -t 01:00:00 -J Bacterial -A PAS2303
```

## Importing fastq files for use in Qiime
```shell
cd /fs/ess/scratch/PAS2303/Your_OSC_ID/Microbiomes/Bacterial
...
mkdir tree_out
apptainer run --writable-tmpfs ../Software/Qiime2.sif qiime alignment mafft --i-sequences deblur_output/representative_sequences.qza --p-n-threads 28 --o-alignment tree_out/rep_seqs_filt_aligned.qza
...
# With metadata, copy metadata file from here: cp /fs/ess/PAS2303/HCS7004_Files/Microbiomes/Metadata.txt The actual datasets are in Zoe's GitHub (https://github.com/zoemigicovsky/cool_climate_grape_microbiome).
apptainer run --writable-tmpfs ../Software/Qiime2.sif qiime diversity alpha-rarefaction --i-table deblur_output/table_filt.qza --p-max-depth 6000 --p-steps 20 --i-phylogeny tree_out/rep_seqs_filt_aligned_masked_tree_rooted.qza --m-metadata-file Metadata.txt --o-visualization rarefaction_curves.qzv
```
## Taxonomy assignment
```shell
mkdir Classifiers
...
wget https://data.qiime2.org/2022.8/common/silva-138-99-515-806-nb-classifier.qza -O Classifiers/silva_16S_515F_806R.qza
